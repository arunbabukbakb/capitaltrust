...
export default function Sidebar({
  ...
  return (
    <>
      ...
      <aside className={`h-full w-64 fixed left-0 top-0 border-r border-slate-200 bg-white shadow-sm flex flex-col py-8 z-50 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
        {/* Brand Identity Header */}
        <div className="px-6 mb-8 flex justify-between items-start">
          <div>
            <h1 className="text-xl font-bold font-headline tracking-tight text-slate-900 flex items-center gap-2">
          ...
          <button onClick={onClose} className="lg:hidden p-1.5 -mr-2 text-slate-400 hover:text-slate-900 hover:bg-slate-100 rounded-lg transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          ...
          <nav className="space-y-1">
            {loadingMenus ? (
              <div className="px-6 py-3 text-xs text-slate-400">Loading menu...</div>
            ) : (
              menus.map((item) => {
                  ...
                  return (
                    <div key={item.id}>
                      <button
                        ...
                        className={`w-full flex justify-between items-center gap-4 px-6 py-3 text-sm font-medium transition-all duration-200 border-r-4 ${isParentActive
                            ? 'text-emerald-700 bg-emerald-50 border-emerald-600 font-bold'
                      ...
                      {isMenuOpen && (
                        <div className="pl-10 py-1 space-y-1 bg-slate-50/50">
                          {item.children.map((child: any) => (
                            <button
                              key={child.id}
                              onClick={() => { onNavigate(child.path!); onClose(); }}
                              className={`w-full text-left flex items-center gap-3 px-4 py-2 text-xs rounded-md ${currentView === child.path
                                  ? 'font-bold text-emerald-800 bg-emerald-100/80'
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-4 px-6 py-3 text-sm font-medium transition-all duration-200 border-r-4 ${currentView === item.path
                        ? 'text-emerald-700 bg-emerald-50 border-emerald-600 font-bold'
                        : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50 border-transparent'
                      }`}
                  >
          ...
          <div className="px-6 py-4 mt-6 border-t border-slate-100">
            <button
              onClick={() => {
                onNavigate('/fund-collection');
                onClose();
              }}
              className="w-full py-3 bg-slate-950 text-white rounded-lg font-medium text-xs tracking-wider uppercase hover:bg-slate-900 transition-all active:scale-[0.98] flex items-center justify-center gap-2"
            >
            ...
            <div className="mt-6 space-y-2">
              {deferredPrompt && (
                <button
                  ...
                  className="w-full flex items-center gap-4 text-xs font-semibold text-emerald-600 hover:text-emerald-700 transition-colors cursor-pointer"
                >
              ...
              <button
                ...
                className="w-full flex items-center gap-4 text-xs font-semibold text-slate-500 hover:text-slate-900 transition-colors"
              >
              ...
              <button
                ...
                className="w-full flex items-center gap-4 text-xs font-semibold text-slate-500 hover:text-rose-600 transition-colors"
              >
