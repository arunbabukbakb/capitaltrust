  ...
  return (
    <div className="space-y-4 sm:space-y-8 animate-fade-in text-slate-900 mt-16 sm:mt-20 mb-5 px-3 sm:px-0">
      ...
      <div className="flex justify-between items-center gap-3">
        <div>
          <h3 className="text-lg sm:text-2xl font-bold font-headline dark:text-slate-100">{t('loanPage.loanEntry')}</h3>
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5 hidden sm:block">
            {t('loanPage.loanEntrySub')}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-8">
        <form onSubmit={handleSubmit} className="lg:col-span-8 bg-white rounded-xl p-4 sm:p-6 border border-slate-200 shadow-sm space-y-4 sm:space-y-6">
          <section className="space-y-3 sm:space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 sm:gap-4">
              <div className="sm:col-span-3">
                <label className="block text-[8px] sm:text-[10px] uppercase font-bold tracking-wider text-slate-500 mb-1 sm:mb-1.5">{t('loanPage.principalAmount')}</label>
                <input
                  required
                  type="number"
                  min="1"
                  ...
                  className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs sm:text-sm font-bold text-slate-900 dark:text-slate-100 focus:outline-none text-tnum"
                />
              </div>
              <div className="sm:col-span-2">
                <label className="block text-[8px] sm:text-[10px] uppercase font-bold tracking-wider text-slate-500 mb-1 sm:mb-1.5">{t('loanPage.tenureMonths')}</label>
                <input
                  required
                  type="number"
                  min="1"
              ...
              <div className="sm:col-span-3">
                <label className="block text-[8px] sm:text-[10px] uppercase font-bold tracking-wider text-slate-500 mb-1 sm:mb-1.5">{t('loanPage.startDate')}</label>
                <input
                  required
                  type="date"
                  value={startDate}
              ...
              <div className="sm:col-span-2">
                <label className="block text-[8px] sm:text-[10px] uppercase font-bold tracking-wider text-slate-500 mb-1 sm:mb-1.5">{t('loanPage.endDate')}</label>
                <input
                  readOnly
                  type="date"
                  value={endDate}
                  className="w-full px-3 py-2 bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg text-xs sm:text-sm font-bold text-slate-500 dark:text-slate-400 text-tnum"
                />
              </div>
              <div className="sm:col-span-2">
                <label className="block text-[8px] sm:text-[10px] uppercase font-bold tracking-wider text-slate-500 mb-1 sm:mb-1.5">{t('loanPage.status')}</label>
                <select
                  value={status}
                  onChange={(e) => setStatus(e.target.value as LoanStatus)}
                  className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs sm:text-sm font-bold text-slate-900 dark:text-slate-100 focus:outline-none cursor-pointer"
                >
                  <option value="Pending">{t('loanPage.pending')}</option>
                  <option value="Active">{t('loanPage.active')}</option>
                  <option value="Closed">{t('loanPage.closed')}</option>
          ...
          <section className="space-y-3 sm:space-y-4 border-t border-slate-100 pt-4 sm:pt-6">
            <div className="flex items-center justify-between gap-4">
              <div>
                <h4 className="text-sm sm:text-base font-bold font-headline dark:text-slate-100">{t('loanPage.groupMembersShare')}</h4>
                <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 mt-0.5">Single loans use one member. Group loans split the amount.</p>
              </div>
              {loanType === 'Group' && (
                <button
                  type="button"
                ...
                >
                  ...
                  <span>{t('loanPage.addMember')}</span>
                </button>
              )}
            </div>
